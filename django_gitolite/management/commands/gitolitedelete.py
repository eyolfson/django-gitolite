...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('repo_path', nargs='+', type=str)

    def handle(self, **options):
        for repo_path in options['repo_path']:
            try:
                repo = Repo.objects.get(path=repo_path)
                user = repo.creator
                if user == None:
                    msg = 'Repo "%s" is not a wildcard.' 
                    raise CommandError(msg.format(repo_name))
